...
tables <- tibble::tribble(
  ...
  "measurement", "nniDTciiniinninicicc",
  c("measurement_id", "person_id", "visit_occurrence_id"),
  c("measurement_id"),
...
for (i in 1:nrow(tables)){
  ...
  get_data <- function(){
    data <<- vroom::vroom(paste0(prefix, table$table_name, ".csv"), col_types = table$col_types)
  ...
  if (table$table_name == "observation") get_data()
  ...
  # cat(i18n$t("success_importing_dataset"))
}