...
observeEvent(input$variable_num_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$variable_num_%widget_id%"))
...
sapply(1:10, function(i){
    ...
    observeEvent(input[[paste0("colour_pal_", i, "_%widget_id%")]], {
        %req%
        if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$colour_pal_", i, "_%widget_id%"))
        ...
        tryCatch({
            ...
            output[[paste0("colour_ui_", i, "_%widget_id%")]] <- renderUI({
                %req%
                if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - output$colour_ui_", i, "_%widget_id%"))
    ...
    observeEvent(input[[paste0("variable_", i, "_%widget_id%")]], {
        %req%
        if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$variable_", i, "_%widget_id%"))
...
observeEvent(m$create_plot_trigger_%widget_id%, {
    ...
    tryCatch({
        ...
        if (create_plot_type == "show_plot") output$dygraph_%widget_id% <- dygraphs::renderDygraph({
            %req%
            if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - output$dygraph_%widget_id%"))
...
observeEvent(input$run_plot_at_patient_changeover_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$run_plot_at_patient_changeover_%widget_id%"))
...
observeEvent(input$run_code_at_patient_changeover_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$run_code_at_patient_changeover_%widget_id%"))
