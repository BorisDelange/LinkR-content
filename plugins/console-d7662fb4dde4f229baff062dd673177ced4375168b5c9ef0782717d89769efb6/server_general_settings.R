...
#         sapply(c("show_saved_file", "figure_and_settings_side_by_side", "run_code_at_patient_update", "run_code_at_settings_file_load"), function(name){
        sapply(c("show_saved_file", "figure_and_settings_side_by_side", "run_code_at_settings_file_load"), function(name){
            toggle_value <- 0L
        ...
        new_data <-
            new_data %>%
            dplyr::transmute(
                id = get_last_row(m$db, "widgets_options") + 1:4, widget_id = %widget_id%, person_id = NA_integer_, link_id,
