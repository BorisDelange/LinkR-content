...
# Generate Plotly histogram code for admission trends
generate_admissions_timeline_code_%widget_id% <- function(indicator = "admission_timeline", indicator_scope = "hospitalization", hospital_units = c(), title = "", x_label = "", y_label = "", nb_bins = "") {
    ...
    code_lines <- c(code_lines,
        ...
        "}",
        "",
        "# Function to translate month names based on language",
        "translate_month_label <- function(date_start, date_end, language) {",
        "    # Calculate time difference to determine appropriate formatting",
        "    time_diff <- as.numeric(difftime(date_end, date_start, units = 'hours'))",
        "    ",
        "    if (time_diff <= 48) {",
        "        # For periods <= 48 hours, show hour format",
        "        start_date <- format(date_start, '%d/%m/%Y')",
        "        end_date <- format(date_end, '%d/%m/%Y')",
        "        start_year <- format(date_start, '%Y')",
        "        end_year <- format(date_end, '%Y')",
        "        ",
        "        if (start_date == end_date) {",
        "            # Same date: show full date for start, only time for end",
        "            start_formatted <- format(date_start, '%d/%m/%Y %H:%M')",
        "            end_formatted <- format(date_end, '%H:%M')",
        "        } else if (start_year == end_year) {",
        "            # Same year: remove year from start",
        "            start_formatted <- format(date_start, '%d/%m %H:%M')",
        "            end_formatted <- format(date_end, '%d/%m/%Y %H:%M')",
        "        } else {",
        ...
        "        }",
        "        ",
        "        return(paste0(start_formatted, ' - ', end_formatted))",
        "    } else if (time_diff <= 24*7) {",
        "        # For periods <= 1 week, show day format",
        "        start_year <- format(date_start, '%Y')",
        "        end_year <- format(date_end, '%Y')",
        "        ",
        "        if (start_year == end_year) {",
        "            start_formatted <- format(date_start, '%d/%m')",
        "            end_formatted <- format(date_end, '%d/%m/%Y')",
        "        } else {",
        "            start_formatted <- format(date_start, '%d/%m/%Y')",
        "            end_formatted <- format(date_end, '%d/%m/%Y')",
        "        }",
        "        ",
        "        return(paste0(start_formatted, ' - ', end_formatted))",
        "    } else {",
        "        # For longer periods, show month format with translation",
        "        start_year <- format(date_start, '%Y')",
        "        end_year <- format(date_end, '%Y')",
        "        ",
        "        if (language == 'fr') {",
        "            months_fr <- c('Jan' = 'Jan', 'Feb' = 'Fev', 'Mar' = 'Mar', 'Apr' = 'Avr',",
        "                          'May' = 'Mai', 'Jun' = 'Juin', 'Jul' = 'Juil', 'Aug' = 'Aou',",
        "                          'Sep' = 'Sep', 'Oct' = 'Oct', 'Nov' = 'Nov', 'Dec' = 'Dec')",
        "            ",
        "            if (start_year == end_year) {",
        "                start_formatted <- format(date_start, '%b')",
        "                end_formatted <- format(date_end, '%b %Y')",
        "            } else {",
        "                start_formatted <- format(date_start, '%b %Y')",
        "                end_formatted <- format(date_end, '%b %Y')",
        "            }",
        "            ",
        "            # Apply French translations",
        "            for (en_month in names(months_fr)) {",
        "                start_formatted <- gsub(en_month, months_fr[en_month], start_formatted)",
        "                end_formatted <- gsub(en_month, months_fr[en_month], end_formatted)",
        "            }",
        "            ",
        "            return(paste0(start_formatted, ' - ', end_formatted))",
        "        } else {",
    ...
    if (indicator_scope == "hospitalization") {
        code_lines <- c(code_lines,
            ...
            "    ",
            "    # Assign each date to a bin (right = FALSE ensures no overlap: [start, end) intervals)",
            "    timeline_data <- timeline_data %>%",
            "        dplyr::mutate(",
            "            bin_start = cut(admission_date, breaks = date_breaks, include.lowest = TRUE, right = FALSE, labels = FALSE),",
            "            bin_start_date = date_breaks[bin_start],",
            "            bin_end_date = date_breaks[bin_start + 1]  # End date is excluded from interval",
            "        ) %>%",
            "        dplyr::filter(!is.na(bin_start))",
            "    ",
            "    # Aggregate by bins",
            "    histogram_data <- timeline_data %>%",
            "        dplyr::group_by(bin_start_date, bin_end_date) %>%",
            "        dplyr::summarise(",
            ...
            "        dplyr::mutate(",
            "            bin_label = sapply(1:nrow(.), function(i) {",
            "                translate_month_label(bin_start_date[i], bin_end_date[i], m$language)",
            "            }),",
            "            # Create tooltip text with formatted dates (show actual included period: subtract 1 minute from end)",
            "            tooltip_period = paste0(",
            "                i18np$t(\"start_date\"), \" : \", format(bin_start_date, \"%d-%m-%Y\"), \" 00:00<br>\",",
            "                i18np$t(\"end_date\"), \" : \", format(as.POSIXct(paste(bin_end_date, \"00:00:00\")) - 60, \"%d-%m-%Y %H:%M\")",
            "            ),",
            "            tooltip_text = paste0(",
            "                tooltip_period, \"<br><br>\",",
            "                i18np$t(\"admission_count_tooltip\"), \" : \", admission_count",
            "            )",
            "        )",
            "} else {",
            ...
            "        bin_end_date = as.Date(character(0)),",
            "        admission_count = numeric(0),",
            "        tooltip_text = character(0)",
            "    )",
            "}",
            ""
        )
        ...
        code_lines <- c(code_lines,
            ...
            "    histogram_data <- timeline_data %>%",
            "        dplyr::group_by(bin_start_date, bin_end_date, care_site_name) %>%",
            "        dplyr::summarise(",
            "            admission_count = dplyr::n(),",
            "            .groups = \"drop\"",
            "        ) %>%",
            ...
            "            unit_details = paste0(care_site_name, \": \", admission_count, collapse = \"<br>* \"),",
            "            .groups = \"drop\"",
            "        ) %>%",
            "        dplyr::arrange(bin_start_date) %>%",
            "        dplyr::mutate(",
            ...
            "                translate_month_label(bin_start_date[i], bin_end_date[i], m$language)",
            "            }),",
            "            # Create tooltip text with formatted dates and unit details (show actual included period: subtract 1 minute from end)",
            "            tooltip_period = paste0(",
            "                i18np$t(\"start_date\"), \" : \", format(bin_start_date, \"%d-%m-%Y\"), \" 00:00<br>\",",
            "                i18np$t(\"end_date\"), \" : \", format(as.POSIXct(paste(bin_end_date, \"00:00:00\")) - 60, \"%d-%m-%Y %H:%M\")",
            "            ),",
            "            tooltip_text = paste0(",
            "                tooltip_period, \"<br><br>\",",
            "                i18np$t(\"admission_count_tooltip\"), \" : \", total_admissions, \"<br><br>\",",
            "                i18np$t(\"admissions_by_unit\"), \" :<br>* \", unit_details",
            "            )",
            "        )",
            "} else {",
            "    histogram_data <- data.frame(",
    ...
    if (indicator_scope == "hospitalization") {
        ...
        code_lines <- c(code_lines,
            "    # Histogram with unit breakdown in hover text",
            "    res <- plotly::plot_ly(",
