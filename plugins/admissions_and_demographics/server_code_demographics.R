...
generate_demographics_ui_code_%widget_id% <- function(indicator = "average_age", indicator_scope = "hospitalization", hospital_units = c(), legend_1 = "", legend_2 = "") {
    ...
    if (indicator == "average_age") {
        ...
        if (indicator_scope == "hospitalization") {
            code_lines <- c(code_lines,
                ...
                "",
                "if (total_patients > 0) {",
                generate_demographics_legend_code_%widget_id%(legend_1, legend_2, "i18np$t(\"gender_distribution\")"),
                "    # Create gender breakdown display",
                "    gender_display <- if(nrow(gender_counts) > 0) {",
                "        paste0(gender_counts$concept_name, \": \", gender_counts$n, \" (\", round((gender_counts$n/total_patients)*100, 1), \"%)\", collapse = \"<br>\")",
                "    } else {",
                "        \"\"",
                "    }",
                "    ",
                ...
                paste0("        style = \"", card_container_style, "\","),
                "        div(",
                paste0("            style = \"color: ", gender_color, "; ", icon_style, "\","),
                "            tags$i(class = \"fas fa-venus-mars fa-2x\")",
                "        ),",
                "        div(",
                paste0("            style = \"color: ", gender_color, "; ", value_style, " position: relative; cursor: pointer;\","),
                "            total_patients,",
                paste0("            onmouseover = \"", tooltip_js_show_value, "        \","),
                paste0("            onmouseout = \"", tooltip_js_hide_value, "\","),
                paste0("            div(class = \"", tooltip_class_name, "\", HTML(gender_display), style = \"", tooltip_style, "\")"),
                "        ),",
                "        div(",
                paste0("            style = \"", label_style, "\","),
                "            do.call(div, legend_div_content)",
            ...
            code_lines <- c(code_lines,
                ...
                "",
                "if (total_patients > 0) {",
                generate_demographics_legend_code_%widget_id%(legend_1, legend_2, "i18np$t(\"gender_distribution\")", "i18np$t(\"in_selected_units\")"),
                "    # Create gender breakdown display",
                "    gender_display <- if(nrow(gender_counts) > 0) {",
                "        paste0(gender_counts$concept_name, \": \", gender_counts$n, \" (\", round((gender_counts$n/total_patients)*100, 1), \"%)\", collapse = \"<br>\")",
                "    } else {",
