...
observeEvent(input$current_tab_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$current_tab_%widget_id%_run_selection"))
...
observeEvent(input$script_code_%widget_id%_run_selection, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$script_code_%widget_id%_run_selection"))
...
observeEvent(input$script_code_%widget_id%_run_all, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$script_code_%widget_id%_run_all"))
...
observeEvent(input$run_code_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$run_code_%widget_id%"))
...
observeEvent(m$script_code_trigger_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer m$script_code_trigger_%widget_id%"))
    ...
    tryCatch({
        ...
        if (input$script_type_%widget_id% == "r"){
            ...
            output$r_script_result_%widget_id% <- renderText({
                %req%
                if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - output$r_script_result_%widget_id%"))
        ...
        if (input$script_type_%widget_id% == "rmarkdown"){
            ...
            tryCatch({
                ...
                output$rmarkdown_script_result_%widget_id% <- renderUI({
                    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - output$rmarkdown_script_result_%widget_id%"))
        ...
        if (input$script_type_%widget_id% == "plot"){
            ...
            output$plot_script_result_%widget_id% <- renderPlot({
                %req%
                if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - output$plot_script_result_%widget_id%"))
...
observeEvent(input$script_code_%widget_id%_save, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$script_code_%widget_id%_save"))
...
observeEvent(input$save_code_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$save_code_%widget_id%"))
...
observeEvent(m$save_code_trigger_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer m$save_code_trigger_%widget_id%"))
...
observeEvent(input$script_choice_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$script_choice_%widget_id%"))
...
#     %req%
#     if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$plot_width_%widget_id%"))
#     
...
observeEvent(m$reload_dt_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer m$reload_dt_%widget_id%"))
...
observeEvent(input$add_script_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$add_script_%widget_id%"))
...
observeEvent(input$scripts_management_datatable_%widget_id%_cell_edit, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$scripts_management_datatable_%widget_id%_cell_edit"))
...
observeEvent(input$save_scripts_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$save_scripts_%widget_id%"))
...
observeEvent(input$deleted_pressed_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$deleted_pressed_%widget_id%"))
...
observeEvent(input$delete_scripts_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$delete_scripts_%widget_id%"))
...
output$delete_confirm_%widget_id% <- shiny.fluent::renderReact({
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - output$delete_confirm_%widget_id%"))
...
observeEvent(input$scripts_hide_dialog_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$scripts_hide_dialog_%widget_id%"))
...
observeEvent(input$scripts_delete_canceled_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$scripts_delete_canceled_%widget_id%"))
...
observeEvent(input$scripts_delete_confirmed_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$scripts_delete_confirmed_%widget_id%"))
...
observeEvent(input$script_type_%widget_id%, {
    %req%
    if (debug) print(paste0(now(), " - mod_", id, " - widget_id = %widget_id% - observer input$script_type_%widget_id%"))
