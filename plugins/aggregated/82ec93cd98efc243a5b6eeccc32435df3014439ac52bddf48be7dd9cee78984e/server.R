...
observeEvent(input$show_stats_%widget_id%, {
    ...
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$show_stats_%widget_id%"))
...
observeEvent(input$show_params_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$show_params_%widget_id%"))
    ...
    tryCatch({
        ...
        # Reload plot
        m$render_results_%widget_id% <- now() 
    }, error = function(e) cat(paste0("\n", now(), " - ", toString(e))))
})
...
observeEvent(input$side_by_side_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$side_by_side_%widget_id%"))
...
observeEvent(input$check_all_hospital_units_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$check_all_hospital_units_%widget_id%"))
...
observeEvent(input$uncheck_all_hospital_units_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$uncheck_all_hospital_units_%widget_id%"))
...
observeEvent(input$var_choice_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$var_choice_%widget_id%"))
...
observeEvent(input$show_plot_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$show_plot_%widget_id%"))
...
observeEvent(m$render_results_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer r$render_results_%widget_id%"))
