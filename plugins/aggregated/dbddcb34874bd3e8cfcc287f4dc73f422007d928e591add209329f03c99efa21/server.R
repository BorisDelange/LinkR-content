...
observeEvent(input$group_data_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$group_data_%widget_id%"))
...
observeEvent(input$group_by_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$group_by_%widget_id%"))
...
observeEvent(input$plot_function_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$plot_function_%widget_id%"))
...
observeEvent(input$plot_function_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$plot_function_%widget_id% (only once)"))
...
observeEvent(input$bins_type_%widget_id%, {
     %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$bins_type_%widget_id%"))
...
observeEvent(input$show_%widget_id%, {
    ...
    tryCatch({
        ...
        output$plot_output_%widget_id% <- renderPlot({
            %req%
            if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - output$plot_output_%widget_id%"))
            ggplot2::ggplot()
        })
...
observeEvent(input$colour_pal_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$colour_pal_%widget_id%"))
    ...
    tryCatch({
        ...
        output$colour_ui_%widget_id% <- renderUI({
            %req%
            if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - output$colour_ui_%widget_id%"))
...
observeEvent(input$run_plot_at_script_launch_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$run_plot_at_script_launch_%widget_id%"))
...
observeEvent(input$run_code_at_script_launch_%widget_id%, {
    %req%
    if (debug) cat(paste0("\n", now(), " - mod_", id, " - widget_id = %widget_id% - observer input$run_code_at_script_launch_%widget_id%"))
