...
patients <- function(){
  vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/icu/icustays.csv.gz") %>% 
    ...
    dplyr::ungroup() %>%
    dplyr::inner_join(
      vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/core/patients.csv.gz") %>% 
...
import_datamart(output = output, r = r, datamart_id = %datamart_id%, data = patients(), type = "patients", save_as_csv = TRUE, rewrite = FALSE, language = "FR")
...
import_datamart(output = output, r = r, datamart_id = %datamart_id%, data = stays(), type = "stays", save_as_csv = TRUE, rewrite = FALSE, language = "FR")
...
labs_vitals <- function(){
  vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/icu/chartevents.csv.gz") %>%
    ...
    dplyr::inner_join(patients() %>% dplyr::select(patient_id), by = "patient_id") %>%
    dplyr::bind_rows(
      vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/hosp/labevents.csv.gz") %>%
        dplyr::transmute(patient_id = subject_id, thesaurus_name = "MIMIC-IV", item_id = itemid, datetime_start = charttime, datetime_stop = "", 
          value, value_num = valuenum, unit = valueuom, comments = "") %>%
        dplyr::inner_join(patients() %>% dplyr::select(patient_id), by = "patient_id")
    ) %>%
    dplyr::bind_rows(
      vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/icu/datetimeevents.csv.gz") %>%
        dplyr::transmute(patient_id = subject_id, thesaurus_name = "MIMIC-IV", item_id = itemid, datetime_start = charttime, datetime_stop = "", 
    ...
    dplyr::bind_rows(
      vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/icu/outputevents.csv.gz") %>%
        dplyr::transmute(patient_id = subject_id, thesaurus_name = "MIMIC-IV", item_id = itemid, datetime_start = charttime, datetime_stop = "", 
...
import_datamart(output = output, r = r, datamart_id = %datamart_id%, data = labs_vitals(), type = "labs_vitals", save_as_csv = TRUE, rewrite = FALSE, language = "FR")

########## ORDERS ##########
orders <- function(){
  vroom::vroom("https://physionet.org/files/mimic-iv-demo/1.0/icu/inputevents.csv.gz") %>%
...
import_datamart(output = output, r = r, datamart_id = %datamart_id%, data = orders(), type = "orders", save_as_csv = TRUE, rewrite = FALSE, language = "FR")